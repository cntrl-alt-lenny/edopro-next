...
- This seat is normally the **primary interactive session**, not a subagent.
  Nothing auto-loads the contract — a session starts here and is pointed at
  these files. The frontmatter exists so it can also be dispatched for a
  narrowly scoped planning sub-task; that is the secondary use.
- Work from the primary checkout. Other concurrently-active seats have their
  own.
