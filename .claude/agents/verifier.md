...
- The `tools:` line above is applied only when this file is loaded as a
  ...
  session in its own checkout — agent-file frontmatter is not applied at all,
  so that line restricts nothing there. The read-only discipline is then the
  role's own to keep. `Bash` is listed because reproducing evidence yourself is
  the job: run git, builds and tests.
- This is the seat most likely to be run on a different tool, which is why the
  contract is self-contained enough to paste whole. Nothing depends on this
  adapter existing.
