...
3. Check `gh pr list` for open PRs, and note for each whether its body still
   carries `DO NOT MERGE`.
4. Check `docs/briefs/active.md`: is a brief queued, in progress (a branch
   matching it exists locally or on `origin`), or is it the placeholder?
5. If `.git/agent-inbox/*.md` exist, check their timestamps. Anything newer
