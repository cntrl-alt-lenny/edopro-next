...
Accepted by Brain and merged as PR #31 (merge `45bb2f33`) at head
`a1c67be9e190d0f06dff988ca45651a076b04a41`, base
`113e2c7f44db9d4a8659e1217488f7b40155307a`, after two passes. The deck
builder now shows advisory legality from `policy::validate_deck()`. The user
selects a banlist ("No banlist" is distinct from "N/A") and a named ruleset,
"Standard OCG/TCG". ADR 0010 records the decision.

**Pass 1, `259b7bcd`, Builder and Verifier in Antigravity (Gemini).** The
Verifier returned ACCEPT with no findings. Brain rejected the pass on three
problems, all found by reading source:

- **S1.** ADR 0010 cited upstream constants that exist nowhere in `gframe/`
  (`MIN_MAIN_DECK`, `MAX_MAIN_DECK`, `MAX_EXTRA_DECK`, `MAX_SIDE_DECK`,
  `ALLOWED_OCG_TCG = 0x3`, `RULE_RITUAL_IN_EXTRA`), and lines that hold
  unrelated code.
- **S2.** The ruleset's card pool (`OcgAndTcg`, index 2) was presented as
  upstream-grounded, but upstream's stored default is index 3
  (`ALLOWED_CARDS_WITH_PRERELEASE`; `game_config.inl:21` → `game.cpp:1160` →
  `duelclient.cpp:229` → `generic_duel.cpp:381` → `deck_manager.h:32-37`).
- **S3.** Under the default "No banlist", the screen said "Deck is legal"
  while `policy/` (faithfully to upstream's null `LFList*`) skips the
  card-scope, section-placement and copy-limit checks.

**Pass 2, `a1c67be9`, Builder and Verifier in Claude Code (Sonnet 5).**

- S1-S3 were resolved. The ADR's field table now carries verbatim quotes.
  Decision 1a records the card-pool divergence plainly. The adapter now says
  which checks are off whenever no banlist is selected, and `policy/` is
  unchanged.
- The Verifier's three SHOULD FIX items were adjudicated by Brain:
  - **Upheld, small:** ADR 0010 Decision 3 cites "`ImportDeck`,
    `deck_manager.cpp:136-146`". Those lines are `DeckManager::TypeCount`.
    The function is `DeckBuilder::ImportDeck`, `gframe/deck_con.cpp:136-146`.
  - **Upheld as a coverage gap:** no committed test pins S3's second example
    (an Extra Deck monster in Main under "No banlist"). The Verifier showed
    the behaviour is correct.
  - **Overruled:** "`ocgcore/ocgapi_constants.h` does not exist". It does,
    with exactly the cited lines. The Verifier's worktree had the `ocgcore`
    submodule uninitialised. Brain has since initialised it in both seat
    worktrees.

**Acceptance conditions, confirmed at the literal head.** The Verifier
reviewed `a1c67be9` (task `015-verify-deck-builder-legality-ui-2`). Brain
independently checked the blocking and citation claims in both passes. All
required checks were green (12 success, 1 conditional matrix entry skipped;
merge state CLEAN). The change is inside routine scope.

**Carried to the next round:** the `ImportDeck` citation and the missing
test.
