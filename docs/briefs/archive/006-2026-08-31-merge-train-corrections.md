...
A corrective round that deliberately created no branch: both corrections went
onto branches that already had open PRs.

- **Correction A** (`card-search.md` §1.4) delivered as `7639bf2d` on PR #19.
- **Correction B** (the QML mirror) delivered as `2f325e15` on PR #21.

Both merged 2026-09-01, in `436f4265` and `f1eb9d66` respectively.

**Outcome: accepted, and Correction B exceeded the brief.** The brief's
required investigation 1 asked whether the mirror could be avoided entirely
rather than refreshed better; it could, and it was. `NO_CACHEGEN` removes the
cause both earlier fixes were working around, so the symlink-versus-copy
distinction that produced the staleness no longer exists on any platform. That
is a better outcome than the refresh mechanism the brief was prepared to
accept.

Correction A's Builder re-derived all five comparators from
`gframe/data_manager.cpp` and confirmed Brain's table matched source on every
row, then replaced the single sentence with a per-function list rather than
force one shape onto three genuinely different ones.
