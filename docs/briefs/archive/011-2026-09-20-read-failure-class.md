...
Accepted by Brain and merged as PR #24 (merge `823fa679`) at head
`45be9de678f0d07a361cdc452f9ca6a4fd50a380`, base
`d3b458bb803af6df22a20574a495bd1680b57ebc`. Five delivered heads were reviewed
in total: `3b15ad56` on macOS, then `338fe1e8`, `797a1d86`, `2a3f3e43` and
`45be9de6` on Windows 11 / MSVC 19.44 after the agent loop moved machines.

**Acceptance conditions, confirmed at the literal head.** The Verifier
reviewed `45be9de6` itself (task `011-verify-read-failure-class-4`). Brain
independently checked every BLOCKER and UNPROVEN CLAIM raised against every
reviewed head. All required checks were green at that head (12 success, 1
conditional matrix entry skipped), and the change is inside the
routine-acceptance scope.

**How each Windows pass was adjudicated.**

- `338fe1e8` — the first-ever MSVC compile of the Win32 branch was clean
  under `/W4 /WX`. The Verifier's BLOCKER — `std::filesystem::status()` on
  `\.\pipe\…` connects as a client, and a busy pipe falls through to
  `ifstream` — was re-derived by Brain. Brain reproduced the connection and
  the fall-through. It did not reproduce a hang, and rated the defect a
  narrow race rather than an observed one. Still not accepted: a known path
  to a blocking read, in the round meant to remove blocking reads. The
  Verifier's claim that the PR body falsely said loader behaviour was
  unchanged was overruled: the sentence described only the latest corrective
  step, whose `.cpp` files were byte-identical. It was rewritten anyway,
  because the whole range is what merges.
- `797a1d86` — the Verifier returned no BLOCKER. Brain found that a
  two-prefix string guard fixed the instance, not the class.
  `\.\GLOBALROOT\Device\NamedPipe\…` hung `load_ydk` indefinitely in 3 of 3
  runs, and three UNC spellings still took the busy fall-through, while the
  architecture document claimed the class closed "by construction".
- `2a3f3e43` — the Verifier's two BLOCKERs were both reproduced by Brain.
  UNC spellings against a server that disconnects immediately and re-listens
  returned `ok=true` with an empty deck, which is Brief 008's original defect
  class. `CON` blocked waiting for console input, which was a regression.
  R11 then stated the underlying constraint as the correction: the object
  whose type decides the load must be the object that is read.
- `45be9de6` — Windows now opens, classifies and reads one native handle. On
  every failure it returns without a second read path. Brain re-probed 80
  loads: five pipe spellings, holding and immediately-disconnecting servers,
  duplex and inbound pipes, four runs each. None succeeded and none hung, and
  `CON` returned promptly with `failed to read file`. The Verifier's four
  UNPROVEN CLAIMs were adjudicated as follows. Two were upheld as
  documentation imprecision (below). One was upheld as trivial and not
  merged: the Builder report's test counts were one low per executable. One
  was overruled: the report's line naming `2a3f3e43` recorded the required
  starting-state check, not final-head evidence.

**Known imprecision merged, to fix in a later round.**
`docs/architecture/read-failure-class.md` says the tested states show "no
tested spelling reaches a second path open". That is literally false. On
`ERROR_ACCESS_DENIED` the loaders make a second, zero-access classification
open, and UNC spellings were observed connecting to the server more than
once. The invariant that holds is narrower and sufficient: no second open is
ever *read*. The same document's description of the zero-access fallback
reads as if only an inbound pipe triggers it. The code enters it for any
`ERROR_ACCESS_DENIED`; an ACL-denied regular file then correctly reports
`failed to open file`, but that case was not run.

**Not established by this round.** macOS was not exercised after
`3b15ad56`. The POSIX code path was not changed afterwards, and Linux CI ran
the POSIX tests at every head. Windows symlink cases, a pipe server that
denies both native access attempts, and an ACL-denied regular file were not
run. Diagnostics for UNC pipe spellings vary between `failed to open file`
and `failed to read file` depending on the server's instance lifetime; only
"prompt and unsuccessful" is claimed. No replay-harness result is evidence
about duel behaviour.
