...
| Deck builder UI | 🔶 **Core working** | search, explicit Main/Extra/Side editing, `.ydk` open/save, and advisory, non-blocking legality against a visible ruleset and banlist (computed by `policy/`), over a tested Qt adapter; no automatic Main/Extra classification, no artwork, no full keyboard/controller parity — [design](architecture/deck-builder-ui.md), [ADR 0010](adr/0010-deck-builder-ruleset-and-legality-ui.md) |
