...
**This role is vendor-neutral.** This contract is the whole of the role: the
owner may hand it, plus a brief, to any model on any tool. Nothing below
depends on a particular vendor's features. Read this file and your brief the
same way, follow the brief's `MODE:`, and report in the schema below.
...
**Vendor-specific mechanics — model selection, reasoning effort, how a session
is started — live in [`../agents/launching.md`](../agents/launching.md), not
here.** This contract must stay true whichever tool runs it.

Two things about launching are part of the contract, because getting them
wrong corrupts the round rather than merely inconveniencing it:

1. **Run in the Builder worktree** (`.worktrees/builder`), never in Brain's
   checkout. Two sessions sharing one working directory is exactly how a
   sibling project ended up with unrelated commits stacked on a work branch
   before review. See
   [`../agents/worktree-mechanism.md`](../agents/worktree-mechanism.md).
2. **Start from the brief and this contract, in fresh context.** A Builder
   carrying over context from an earlier round, or from Brain's reasoning
   about the brief, is no longer independent — and Brain's adjudication
   assumes it is.

If a tool cannot provide something this contract needs, document the gap for
the owner rather than working around it silently.
